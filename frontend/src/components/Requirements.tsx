...
export default function Requirements({ record, busy, onAssess, onRetry, openTender }: { record: CaseRecord; busy: boolean; onAssess: (submission: File, selected: RequirementsPayload, exclusions: string[], note: string) => void; onRetry: (tender?: File) => void; openTender: (page?: number) => void }) {
  ...
  return <div className="requirements-layout"><section><div className="section-topline"><div><h2>Review tender requirements</h2><p className="muted">Confirm what applies before the bidder is scored.</p></div><span className="count-stamp">{count} INCLUDED</span></div>
    {record.requirements.extraction_method === 'heuristic' && <Notice tone="warning"><strong>Conservative extraction used.</strong> {record.requirements.fallback_reason === 'disabled' ? 'Local model extraction is disabled for this development session.' : 'The requirement model was unavailable or returned invalid output.'} Review these clauses for completeness and applicability.</Notice>}
